...
8-bit ambience/drone engine. `buildGraph(ctx, patch, dest, t0, dur)` builds the whole signal path (chip voices → lowpass → volume-envelope gains → tremolo/amp → FX inserts → dry + parallel delay/reverb sends → soft-sat drive → volume) and serves **both** realtime preview (`AudioContext`) and offline bounce (`OfflineAudioContext`) from one codepath, so preview matches the render. Modulation is **assignable slots** routing to volume/cutoff/pitch: **LFOs** (shape sine/tri/square/saw or **drawn** — a drawn cycle becomes a band-limited `PeriodicWave` via DFT) and **drawable envelopes** (a drawn `[0,1]` contour baked into a looping control-rate `AudioBufferSource`, SYNC-able to loop length). Everything is `OscillatorNode`/`AudioBufferSource`/`WaveShaper`/`Biquad` based so it renders natively offline. **FX suite**: bitcrush (WaveShaper staircase), chorus/flanger/phaser (modulated delays + allpass), delay, reverb (convolver) — modulation inserts in series, delay/reverb as parallel sends, each with a dry/wet `insertFx` mix. `renderLoop()` does the **pop-free loop**: render `preRoll + loopLen + crossfade`, discard the pre-roll (so the body is steady-state), then equal-power crossfade the natural continuation just past the loop end back over the loop head, so end→start wraps seamlessly. Output is 16-bit WAV (`bufferToWav`), either downloaded or pushed to RAW FORM via `YuccaSamples.put`. **Patches** save to a named preset bank + auto-restore the working patch (own `localStorage` keys `rawformless_presets`/`rawformless_working` — Raw Formless patches aren't YUCCA-FX presets, so they stay out of the shared bridge), and export/import as `*.rawformless.json` files (`normalizePatch` merges onto defaults so old/partial files don't break). Roadmap below covers what's still stubbed.
